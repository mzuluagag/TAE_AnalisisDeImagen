...
datos$persona<-str_extract(listaArchivos, ".+?(?=_)")
personCod<-unique(str_extract(listaArchivos, ".+?(?=_)"))

for (i in 1:length(personCod)){
  datos$persona<-replace(datos$persona,datos$persona==personCod[i],i)
}

datos$V3<-as.numeric(datos$V3)
datos$persona<-as.numeric(datos$persona)
modTot<-neuralnet(V3~persona+V2,data=datos,hidden=3,act.fct = "logistic",
                  linear.output = FALSE)
predicc<-predict(modTot,datos)
